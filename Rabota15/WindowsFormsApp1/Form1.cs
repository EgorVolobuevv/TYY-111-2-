    ...
    public partial class Kalendar : Form
    {
        ...
        private void dateTimePicker1_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (selectedDate >= firstSemesterStart && selectedDate <= firstSemesterEnd)
            {
                ...
                label1.Text = $"Выбранная дата относится к {2 - (weekNumber % 2)} учебной неделе  первого семестра ({calendarWeekNumber} в календарном году)";
            }
            else if (selectedDate >= secondSemesterStart && selectedDate <= secondSemesterEnd)
            {
                ...
                int calendarWeekNumber = GetCalendarWeekNumber(selectedDate);
                label1.Text = $"Выбранная дата относится к  {2 - (weekNumber % 2)} учебной неделе второго семестра({calendarWeekNumber} в календарном году)";
            }
            else
            {
                label1.Text = "Выбранная дата не относится к учебным неделям";
            }
        }
